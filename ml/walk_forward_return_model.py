...
def main():
    ...
    print("[INFO] Loading walk-forward regression dataset...")
    ...
    print("==============================")
    print("WALK-FORWARD RETURN MODEL")
    print("==============================")
    ...
    while start + TRAIN_SIZE + TEST_SIZE <= len(df):
        ...
        trades = test_df[test_df["predicted_return"] > RETURN_THRESHOLD].copy()
        ...
        train_start = train_df["signal_time"].min()
        train_end = train_df["signal_time"].max()
        test_start = test_df["signal_time"].min()
        test_end = test_df["signal_time"].max()
    ...
    cursor.execute("DROP TABLE IF EXISTS walk_forward_return_results;")

    cursor.execute("""
        CREATE TABLE walk_forward_return_results (
    ...
    cursor.executemany("""
        INSERT INTO walk_forward_return_results (
            window_id,
            train_start,
            train_end,
            test_start,
    ...
    cursor.execute("""
        CREATE OR REPLACE VIEW walk_forward_return_summary AS
        SELECT
    ...
    print("==============================")
    print("WALK-FORWARD RETURN COMPLETE")
    print("==============================")
    print(f"Windows tested: {len(results)}")
    print("Saved table: walk_forward_return_results")
